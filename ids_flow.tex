...
\begin{tikzpicture}[node distance = 3cm, sibling distance=2cm, auto]
    % Place nodes
    \node [decision] (ids) {Intrusion\\Detection\\System}
        child { node (acarm) [tool, left=2cm of ids] {ACARM-ng}}
        child { node (aide) [tool] {AIDE}}
        child { node (bro) [tool] {Bro}}
        child { node (snort) [tool, below=1cm of ids] {Snort}}
        child { node (surcata) [tool] {Surcata}}
        child { node (prelude) [tool] {Prelude\\Hybrid}}
        child { node (samhain) [tool, right=2cm of ids]  {Samhain}};
